    ...
    function init(plot) {
        function drawNumbers(plot, ctx, series){
            if(series.bars.showNumbers){
                ...
                ctx.fillStyle = series.bars.fillColor;

                for(var datapoint in points){
                    if((datapoint - 1) % 3 == 0){
                        var point = { 'x': points[datapoint - 1] + alignOffset, 'y': points[datapoint] }
                        var c = plot.p2c(point);
                        c.top -= 15;
                        ctx.fillText(number_format(points[datapoint].toString(10)), c.left + offset.left, c.top + offset.top)
                    }
                }
            }
        }
